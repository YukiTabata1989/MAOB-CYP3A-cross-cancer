...
# Round 3B: replacement Figure 4 panels and Supplementary Figure S4
# =============================================================================
#
# PURPOSE
#   Rebuild Figure 4A/4B/4C from the denominator-aligned Round 3A outputs and
#   render the pseudo-target calibration as Supplementary Figure S4.
#
# ANALYTICAL BOUNDARY
...
#   - Supplementary Figure S4 reads the frozen Round 3A pseudo-target P values
#     from table 166. This is necessary to audit text sizes and render a fresh
#     vector PDF with embedded-font support; copying the old PDF cannot provide
#     those guarantees.
...
#
# OUTPUTS
#   results/round3B/figures/
#     Figure4A_round3B_denominator_aligned_recurrence_null.{pdf,png}
#     Figure4B_round3B_coverage_sensitivity.{pdf,png}
#     Figure4C_round3B_MAOB_organ_collapsed_sensitivity.{pdf,png}
#     SupplementaryFigureS4_round3B_pseudo_target_calibration_QQ.{pdf,png}
#
#   results/round3B/logs/
#     round3B_progress.log
#     round3B_input_manifest.csv
#     round3B_text_size_audit.csv
#     round3B_figure_dimensions.csv
#     round3B_pdf_device_manifest.csv
#     round3B_integrity_checks.csv
#     round3B_completion_report.txt
...
analysis_id <- "MAOB_CYP3A_tissue_disruption_round3B_figures"
...
round3b_dir <- file.path(project_dir, "results", "round3B")
figure_dir <- file.path(round3b_dir, "figures")
log_dir <- file.path(round3b_dir, "logs")
invisible(lapply(
  c(round3b_dir, figure_dir, log_dir),
  dir.create,
  recursive = TRUE,
  showWarnings = FALSE
))
...
input_files <- c(
  ...
  pseudo_target_values = file.path(
    round3a_table_dir,
...
progress_log_file <- file.path(log_dir, "round3B_progress.log")
input_manifest_file <- file.path(log_dir, "round3B_input_manifest.csv")
text_audit_file <- file.path(log_dir, "round3B_text_size_audit.csv")
figure_spec_file <- file.path(log_dir, "round3B_figure_dimensions.csv")
pdf_device_file <- file.path(log_dir, "round3B_pdf_device_manifest.csv")
integrity_file <- file.path(log_dir, "round3B_integrity_checks.csv")
completion_report_file <- file.path(log_dir, "round3B_completion_report.txt")
session_info_file <- file.path(log_dir, "round3B_sessionInfo.txt")
...
pseudo_target_values <- read_frozen_csv(
  "pseudo_target_values",
  c(
    "target_gene", "minimum_coverage_fraction", "endpoint",
    "empirical_p_plus1_leave_one_out", "randomized_tie_rank_p"
  )
)
...
figure4a_annotations <- recurrence_80 %>%
  ...
  transmute(
    ...
    denominator_label = if_else(
      status == "estimable_denominator_aligned_null",
      paste0("common denominator: ", as.integer(common_denominator_n)),
      ""
    ),
...
plot_figure4a <- ggplot(
  ...
  geom_text(
    data = figure4a_annotations,
    aes(x = gene, y = 1.080, label = denominator_label),
    inherit.aes = FALSE,
    size = 2.65,
    color = "grey20"
  ) +
  geom_text(
    data = figure4a_annotations,
    aes(x = gene, y = 1.020, label = profile_label),
    inherit.aes = FALSE,
    size = 2.65,
    color = "grey25"
  ) +
...
save_plot_pair(
  plot_figure4a,
  "Figure4A_round3B_denominator_aligned_recurrence_null",
  height_mm = 140
)
...
plot_figure4b_main <- ggplot(
  ...
  scale_shape_manual(
    values = c(
      "Recurrent loss (primary)" = 16,
      "Negative median (supplementary)" = 1
    ),
    drop = FALSE
  ) +
  scale_x_continuous(
...
save_plot_grid_pair(
  ...
  heights = c(2.25, 1.15),
  stem = "Figure4B_round3B_coverage_sensitivity",
  height_mm = 190
)
...
plot_figure4c <- ggplot() +
  ...
  labs(
    ...
    caption = "Not-evaluable positions are display locations and do not represent effect size or P value."
  ) +
  theme_manuscript(base_size = 10) +
  theme(
    plot.caption.position = "plot",
...
save_plot_pair(
  plot_figure4c,
  "Figure4C_round3B_MAOB_organ_collapsed_sensitivity",
  height_mm = 125
)

# ---- 6. Supplementary Figure S4: pseudo-target calibration -----------------

log_message("Building Supplementary Figure S4 from frozen pseudo-target P values")

s4_data <- pseudo_target_values %>%
  filter(endpoint == "recurrent_loss") %>%
  select(
    target_gene, minimum_coverage_fraction,
    empirical_p_plus1_leave_one_out, randomized_tie_rank_p
  ) %>%
  pivot_longer(
    cols = c(empirical_p_plus1_leave_one_out, randomized_tie_rank_p),
    names_to = "diagnostic",
    values_to = "observed_p"
  ) %>%
  filter(is.finite(observed_p)) %>%
  group_by(target_gene, minimum_coverage_fraction, diagnostic) %>%
  arrange(observed_p, .by_group = TRUE) %>%
  mutate(expected_uniform_quantile = (row_number() - 0.5) / n()) %>%
  ungroup() %>%
  mutate(
    target_gene = factor(target_gene, levels = target_genes),
    coverage_label = factor(
      paste0(round(100 * minimum_coverage_fraction), "%"),
      levels = paste0(round(100 * coverage_levels), "%")
    ),
    diagnostic = recode(
      diagnostic,
      randomized_tie_rank_p = "Primary: tie-randomized rank",
      empirical_p_plus1_leave_one_out = "Supplementary: conservative +1"
    ),
    diagnostic = factor(
      diagnostic,
      levels = c(
        "Primary: tie-randomized rank",
        "Supplementary: conservative +1"
      )
    )
  )
...
assert_true(
  nrow(s4_data) > 0L &&
    setequal(
      unique(as.character(s4_data$diagnostic)),
      c(
        "Primary: tie-randomized rank",
        "Supplementary: conservative +1"
      )
    ),
  "Supplementary Figure S4 lacks one of the two calibration diagnostics."
)

plot_s4 <- ggplot(
  s4_data,
  aes(
    x = expected_uniform_quantile,
    y = observed_p,
    color = diagnostic,
    shape = diagnostic
  )
  ...
  geom_point(size = 1.05, alpha = 0.72, stroke = 0.5) +
  facet_grid(target_gene ~ coverage_label, drop = FALSE) +
  scale_color_manual(
    values = c(
      "Primary: tie-randomized rank" = "#D55E00",
      "Supplementary: conservative +1" = "#0072B2"
    ),
    drop = FALSE
  ) +
  scale_shape_manual(
    values = c(
      "Primary: tie-randomized rank" = 16,
      "Supplementary: conservative +1" = 1
    ),
    drop = FALSE
  ) +
  ...
  labs(
    ...
    y = "Observed pseudo-target P value",
    color = "Calibration diagnostic",
    shape = "Calibration diagnostic",
    title = "Pseudo-target calibration of the denominator-aligned recurrence null",
    subtitle = wrap_plot_text(
      paste(
        "Tie-randomized ranks are the primary continuous-uniform diagnostic;",
        "conservative +1 empirical P values are shown as supplementary information.",
        "Empty facets were not evaluable."
      ),
      width = 115
    )
  ) +
  ...
  theme(
    ...
    panel.grid.minor = element_blank(),
    plot.margin = margin(t = 7, r = 6, b = 6, l = 6)
  ) +
  guides(
...
save_plot_pair(
  plot_s4,
  "SupplementaryFigureS4_round3B_pseudo_target_calibration_QQ",
  height_mm = 210
)

# ---- 7. Final audits and reproducibility ------------------------------------
...
assert_true(
  n_distinct(figure_specs$figure) == 4L,
  "Exactly four Round 3B figure pairs must be produced."
)
assert_true(
  all(figure_specs$width_mm == publication_width_mm) &&
    all(figure_specs$png_dpi == 300),
  "Every Round 3B figure must be 174 mm wide with a 300-dpi PNG."
)
assert_true(
...
assert_true(
  ...
  "All four PDFs must use a vector embedded-font device."
)
...
output_stems <- c(
  "Figure4A_round3B_denominator_aligned_recurrence_null",
  "Figure4B_round3B_coverage_sensitivity",
  "Figure4C_round3B_MAOB_organ_collapsed_sensitivity",
  "SupplementaryFigureS4_round3B_pseudo_target_calibration_QQ"
)
...
integrity_checks <- tibble(
  check = c(
    ...
    "SupplementaryFigureS4_two_diagnostics_displayed",
  ...
  result = c(
    ...
    all(observed_4c$n_loss_observed == observed_4c$n_loss_expected) &&
      ...
      all(observed_4c$profiles_observed == observed_4c$profiles_expected),
    identical(organ_not_evaluable$minimum_coverage_fraction, c(0.8, 0.9)),
    n_distinct(s4_data$diagnostic) == 2L,
    min(text_audit$fontsize_pt) >= minimum_text_pt,
    all(figure_specs$width_mm == publication_width_mm),
    all(figure_specs$png_dpi == 300),
    nrow(pdf_devices) == 4L && all(pdf_devices$embedded_font_device),
  ...
  detail = c(
    ...
    paste(levels(s4_data$diagnostic), collapse = ";"),
    ...
    "No continuous colour scale appears in the four Round 3B figures",
...
completion_report <- c(
  ...
  paste(
    "Supplementary Figure S4: rebuilt from frozen table 166 values to permit",
    "final-size text audit and vector embedded-font rendering."
  ),
  paste0(
